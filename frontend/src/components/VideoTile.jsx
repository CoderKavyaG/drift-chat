// Fixes bug 6: srcObject imperatively via useRef+useEffect, not as JSX prop
...
export function VideoTile({
  ...
  return (
    <div className="relative w-full h-full bg-[#0a0a0f] rounded-2xl overflow-hidden ring-2 ring-[#F4600C]/30 hover:ring-[#F4600C]/60 transition-all duration-200">
      ...
      {/* Audio activity indicator - glowing circle */}
      {isAudioActive && !isLocal && (
        <div className="absolute top-3 left-3 flex items-center gap-2">
          <div className="relative">
            <div className="w-4 h-4 rounded-full bg-green-500 animate-pulse"></div>
            <div className="absolute inset-0 w-4 h-4 rounded-full bg-green-500 opacity-30 animate-ping"></div>
          </div>
          <span className="text-xs font-semibold text-green-400 uppercase tracking-wider">Audio</span>
        </div>
      )}
    </div>
  );
